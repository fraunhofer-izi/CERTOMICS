...
execute:
  ...
  debug: true
params:
  mapped_kmer_dir: "" #'folder to mapped CARkmer counts'
  unmapped_kmer_dir: "" #'folder to mapped CARkmer counts'
...
has_mapped_kmers <- 
  nzchar(params$mapped_kmer_dir) &&
  dir.exists(params$mapped_kmer_dir) &&
  length(list.files(
    params$mapped_kmer_dir,
    pattern = "\\.CAR_kmer_summary\\.txt$"
  )) > 0

has_unmapped_kmers <- 
  nzchar(params$unmapped_kmer_dir) &&
  dir.exists(params$unmapped_kmer_dir) &&
  length(list.files(
    params$unmapped_kmer_dir,
    pattern = "\\.unmapped_kmc_intersect_summary\\.txt$"
  )) > 0

has_any_kmers <- has_mapped_kmers || has_unmapped_kmers
...
`r if (has_any_kmers)
"### Validation of CAR Transgene Identity"`

```{r, results="asis", echo=FALSE}
if (has_any_kmers) {
  cat("
::: {.callout-note collapse=\"true\"}
## Interpretation of mapped and unmapped CAR-specific k-mer signals:

To validate the identity of the CAR transgene expressed in patient-derived
CAR T cells, we applied a k-mer–based approach using CAR-specific unique 31-mers.

Two complementary analyses were performed:

1. Detection of CAR-specific k-mers in reads mapped to the transgene
   included in the custom reference genome.
2. Detection of CAR-specific k-mers in reads that did not map to the
   transgene reference.

When the **correct** CAR construct is included in the reference, a strong signal is expected in reads mapped to the CAR locus. In addition, CAR-specific k-mers may also be detected in the unmapped read fraction. This is a common feature of scRNA-seq data and reflects alignment sensitivity rather than incorrect construct usage, as a subset of CAR-derived fragments can fail alignment due to low sequence complexity (e.g., polyA tails), sequencing errors, or stringent alignment and filtering criteria.

Conversely, when an **incorrect** CAR construct is used for mapping, CAR-specific signal is largely absent from the mapped reads, while substantial CAR-specific k-mer signal may still be recovered from the unmapped fraction. In this case, unmapped k-mer enrichment indicates the presence of CAR-derived sequences that cannot be aligned to the provided reference, thereby serving as evidence for reference mismatch rather than true absence of CAR expression. Plase note that low-level CAR-specific k-mer counts may also be observed in samples lacking a CAR transgene. This arises because many CAR constructs incorporate endogenous protein-derived regions, such as signal peptides, transmembrane domains, or costimulatory and signaling domains (e.g., CD28, 4-1BB, CD3ζ), whose sequences may be partially shared with endogenous transcripts.
:::
")
}
```
...
```{r}
#| label: plot-car-kmer-analysis
#| fig-cap: "CAR-specific unique k-mer detection across samples in mapped and unmapped reads."
#| echo: false
#| eval: !expr (has_any_kmers)
#| warning: false
#| results: "hide"
...
mapped_all <- do.call(
  rbind,
  lapply(mapped_files, function(f) {
    ...
    sample_id <- sub(
      "\\.CAR_kmer_summary\\.txt$",
      "",
      basename(f)
    )

    # Add / modify columns (base R equivalent of mutate)
    df$Sample <- sample_id
    df$Analysis <- "Mapped"
    df$Kmer_hits <- df$Kmer_matches

    # Select columns (base R equivalent of select)
    df <- df[, c("Sample", "Tested_CAR", "Kmer_hits", "Analysis")]

    df
  })
)

unmapped_files <- list.files(
  params$unmapped_kmer_dir,
  pattern = "\\.unmapped_kmc_intersect_summary\\.txt$",
  full.names = TRUE
)
unmapped_all <- do.call(
  rbind,
  lapply(unmapped_files, function(f) {
    ...
    sample_id <- sub(
      "\\.unmapped_kmc_intersect_summary\\.txt$",
      "",
      basename(f)
    )

    # Base R equivalent of mutate()
    df$Sample <- sample_id
    df$Analysis <- "Unmapped"

    # Base R equivalent of select()
    df <- df[, c("Sample", "Tested_CAR", "Kmer_hits", "Analysis")]
...
glimpse(mapped_all)
glimpse(unmapped_all)
car_kmer_all <- dplyr::bind_rows(
  mapped_all,
  unmapped_all
)

car_kmer_all <- car_kmer_all %>%
  mutate(
    Tested_CAR = str_replace(Tested_CAR, "_CAR_construct$", "")
  )
car_order <- car_kmer_all %>%
  group_by(Tested_CAR) %>%
  summarise(max_hits = max(Kmer_hits), .groups = "drop") %>%
  arrange((max_hits)) %>%
  pull(Tested_CAR)
...
ggplot(
  car_kmer_all,
  aes(
    x = factor(Tested_CAR, levels = car_order),
    y = Kmer_hits,
    fill = Sample
  )
) +
...
if flags["GEX"]:
  sample_names = CAR_quality_plot.plot_read_metrices(read_metrics)
```
...
if (flags$GEX && params$gtf != "none" && !is.null(meta_data)){
  has_gdT <- "is.pure_gdT" %in% colnames(meta_data)
  ...
  all_samples <- total_t_cells$orig.ident
  all_types <- c("CD4", "CD8", "gdT", "Multi")

  car_t_cells_complete <- expand_grid(
    orig.ident = all_samples,
    T_cell_type = all_types
  )
  ...
  car_t_cells_complete <- car_t_cells_complete %>%
    left_join(
      car_t_cells,
      by = c("orig.ident", "T_cell_type")
    ) %>%
    mutate(CAR_count = replace_na(CAR_count, 0))

  car_percent <- car_t_cells_complete %>%
    left_join(total_t_cells, by = "orig.ident") %>%
    mutate(
      percentage = ifelse(total_T > 0, (CAR_count / total_T) * 100, 0),
  ...
  car_percent <- car_percent %>%
    mutate(
      ...
      show_label = ifelse(percentage <= 1, NA, CAR_count)
    )
...
#| fig-cap: "Absolute number of cell types per samples, based on annotation with choosen scGate model."
#| echo: false
#| eval: !expr (!is.null(meta_data))
#| warning: false
#| fig-height: 4
...
custom_colors <- c(
  ...
  "Endothelial"                     ="#E78AC3",   # same color as HSPC as not available in TME_HIres  model
  "Epithelial"                      ="#8C510A",
  "Fibroblast"                      ="#a200ffff",  
  "Macrophage"                      ="#E31A1C"    # same color as Erythrocytes as not available in TME_HIres  model
)
...
#| fig-cap: "Relative number of cell types per samples, based on annotation with choosen scGate model."
#| echo: false
#| eval: !expr (!is.null(meta_data))
#| warning: false
#| fig-height: 4
...
if (flags$VDJT){
  ...
  clone_size_counts <- meta_data_filtered %>%
    ...
    summarise(count = n_distinct(CTstrict), .groups = 'drop')%>%
  mutate(orig.ident = factor(orig.ident, levels = sample_order))
