...
#| fig-cap: "Percentage of CAR-positive cells (count>0) compared to all T cells (CD4, CD8, gd) (based on scGate annotation). Absolute counts of CD4 and CD8 CAR+ T cells are displayed on top of the bars."
#| echo: false
#| warning: false
#| eval: !expr (params$gtf != "none")
#| fig-height: 4
...
if (flags$GEX && params$gtf != "none" && !is.null(meta_data)){
  ...
  car_t_cells <- t_cell_data %>%
    ...
    mutate(T_cell_type = case_when(
      is.pure_CD4T == "Pure" ~ "CD4",
      is.pure_CD8T == "Pure" ~ "CD8",
      TRUE ~ NA_character_
    )) %>%
  ...
  mutate(
    text_color = ifelse(T_cell_type == "CD4", "black", "white")
  )
  ...
  plot_car_proportion <- ggplot(car_percent, aes(x = orig.ident, y = percentage, fill = T_cell_type)) +
    ...
    geom_text(
      aes(label = CAR_count, color = text_color),
      position = position_stack(vjust = 0.5),
      size = 3,
      fontface = "bold"
    ) +
    scale_fill_manual(
      values = c("CD4" = "#6FC1CD", "CD8" = "#005B7F")
    ) +
