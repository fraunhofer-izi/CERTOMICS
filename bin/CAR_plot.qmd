...
for i, car in enumerate(cars):
    plt.bar(x + i * bar_width, count_df[car], width=bar_width, label=car)
...
ax.set_ylabel("Estimated Read Counts", fontsize=24)
ax.set_title("CAR Read Counts per Sample", fontsize=26)
...
ax.legend(title="CAR", fontsize=20, title_fontsize=20)
...
if (flags$GEX && params$gtf != "none" && !is.null(meta_data)){
  ...
  labs(
    title = "CAR+ NK Cells",
    x = "Sample",
    y = "CAR+ NK Cells (% of all NK cells)"
  ) +
  theme_minimal() +
...
if (flags$GEX && params$gtf != "none" && !is.null(meta_data)){
  ...
  labs(
    ...
    x = "Sample",
    y = "CAR+ Macrophages (% of all Macrophages cells)"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme_custom(base_size = 10)
...
if (flags$GEX && params$gtf != "none" && !is.null(meta_data)){
  ...
  labs(
    ...
    x = "Sample",
    y = "CAR+ Dendritic Cells (% of all Dentritic cells)"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme_custom(base_size = 10)
